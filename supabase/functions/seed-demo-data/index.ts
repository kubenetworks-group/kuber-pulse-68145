...
function generatePods(
  ...
  for (let i = 0; i < sysPodCount; i++) {
    ...
    const cpuMilli = randInt(sp.cpuRange[0], sp.cpuRange[1]);
    const memMi    = randInt(sp.memRange[0], sp.memRange[1]);
    const restarts = randInt(sp.restartRange[0], sp.restartRange[1]);
    ...
    pods.push({
      ...
      containers: [{
        name: sp.name, restart_count: restarts,
  ...
  for (let i = 0; i < appPodCount; i++) {
    ...
    pods.push({
      ...
      containers: [{
        name: app, restart_count: restarts,
        ready: phase === "Running",
